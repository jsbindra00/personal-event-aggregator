...
The source adapters use the websites' first-party browser traffic or rendered page data. They do not use official event-platform APIs and never submit credentials through this application. Guild is represented as unavailable because the service closed on 1 October 2024.

Specifically, Meetup is read from its browser GraphQL search traffic, Luma from its browser JSON discovery responses, and Eventbrite from the event-list structured data rendered on its city pages. These are private website contracts rather than supported public APIs, so each source is isolated and reports authentication, rate-limit, network, or contract-drift failures without stopping results from the others.
...
2. Use a source's **Connect** or **Sign in again** action. A dedicated Chrome window opens; sign in on the source website itself.
3. Enter a city or full address, choose the first and last calendar dates, and search. Both dates are inclusive in the selected local time zone.
4. Results appear progressively. Each card links to the original event page. A failed or signed-out source remains isolated from the others.
...
Search tasks start together, with a short abortable browser-start stagger for Luma and Eventbrite because one persistent Chrome profile is shared. Operations against the same source are serialized so concurrent web, API, and MCP searches cannot navigate one another's page.
